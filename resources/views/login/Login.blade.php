    ...
    <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
        ...
        <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
            <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                ...
                @if(session('success'))
                    <div id="successMessage" class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded-lg flex items-center">
                        <img src="https://i.gifer.com/7efs.gif" class="w-10 h-10 mr-3" alt="Success GIF">
                        <p>{{ session('success') }}</p>
                    </div>
                ...
                <form class="space-y-4 md:space-y-6" action="{{ route('login') }}" method="POST">
                    @csrf 
                    <div>
                        <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Your user name</label>
                    ...
                    @if ($errors->has('login'))
                    <p class="text-center" style="color: red;">{{ $errors->first('login') }}</p>
                    @endif
